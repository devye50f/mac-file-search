...
@MainActor final class SearchViewModel: ObservableObject {
    ...
    private var task: Task<Void, Never>?, token: CancellationToken?, generation = UUID()
    private static let locationKey = "SearchLocationPaths"

    init() {
        let saved = UserDefaults.standard.stringArray(forKey: Self.locationKey) ?? []
        locations = (saved.isEmpty ? [NSHomeDirectory()] : saved).map(SearchLocation.init(path:))
    }

    var availableOperators: [Operator] {
        switch field {
        case .size: return [.above, .below, .inclusiveRange]
        case .created, .modified, .dateAdded: return [.before, .after]
        default: return [.contains, .equals, .excludes, .startsWith, .endsWith, .matchesRegex, .doesNotMatchRegex]
        }
    }

    func ensureValidOperator() {
        if !availableOperators.contains(op) { op = availableOperators[0] }
    }

    func chooseLocations() {
        let panel = NSOpenPanel()
        panel.title = "Choose folders or volumes to search"
        panel.prompt = "Add Locations"
        panel.message = "Select one or more folders, user home folders, or mounted volumes. macOS permissions still apply."
        ...
        let existing = Set(locations.map { URL(fileURLWithPath: $0.path).standardizedFileURL.path })
        locations.append(contentsOf: panel.urls
            .map { $0.standardizedFileURL.path }
            .filter { !existing.contains($0) }
            .map(SearchLocation.init(path:)))
        saveLocations()
    }
    ...
    func resetToHome() { locations = [SearchLocation(path: NSHomeDirectory())]; saveLocations() }

    private func saveLocations() {
        UserDefaults.standard.set(locations.map(\.path), forKey: Self.locationKey)
    }

    func runDirect() {
        cancel(); ensureValidOperator(); results = []; coverageDetails = []
        guard !locations.isEmpty else { status = "Add at least one search location"; return }
        ...
        task = Task {
            ...
            coverageDetails = summary.messages
            status = "Examined \(summary.examined) files — \(summary.matched) matches; \(summary.unknown) incomplete; \(summary.skipped) inaccessible/skipped"
            running = false
        }
    }
    ...
    var body: some View {
        NavigationSplitView {
            List {
                Section("Search Locations") {
                    ForEach(model.locations) { location in
                        Label(location.path, systemImage: "folder").lineLimit(2).help(location.path)
                    }.onDelete(perform: model.removeLocations)
                    HStack {
                        Button("Add…", action: model.chooseLocations).accessibilityLabel("Add search locations")
                        Button("Home", action: model.resetToHome).accessibilityLabel("Reset search locations to current home folder")
                    }
                }
            ...
            VStack(spacing: 10) {
                ...
                if !model.coverageDetails.isEmpty {
                    DisclosureGroup("Coverage and access details") {
                        ForEach(Array(model.coverageDetails.enumerated()), id: \.offset) { _, message in
