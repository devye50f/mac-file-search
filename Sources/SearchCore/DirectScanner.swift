        ...
        for location in search.locations {
            ...
            guard let enumerator = fm.enumerator(at: URL(fileURLWithPath: location.path), includingPropertiesForKeys: keys,
                options: location.includeSubfolders ? [] : [.skipsSubdirectoryDescendants], errorHandler: { url, error in
                    issues.append("Cannot access \(url.path): \(error.localizedDescription)")
                    return true
                }) else {
                summary.skipped += 1
                summary.messages.append("Could not enumerate location: \(location.path)")
                continue
            }
            while let url = enumerator.nextObject() as? URL {
                ...
                do {
                    ...
                    // Some providers do not populate isRegularFile even though the URL is a
                    // searchable, non-directory item. Exclude known non-files instead of
                    // silently throwing those provider-backed files away.
            ...
            let accessIssues = issues.take()
            summary.skipped += accessIssues.count
            summary.messages.append(contentsOf: accessIssues.prefix(20))
            if accessIssues.count > 20 { summary.messages.append("\(accessIssues.count - 20) additional access errors") }
        }
...
private final class ScanIssues: @unchecked Sendable {
    ...
    func take() -> [String] { lock.lock(); defer { lock.unlock() }; return values }
}
